...
# --- ROBUST MULTI-BACKEND SEARCH TOOL ---
@tool
def web_search_tool(query: str, **kwargs) -> str:
    ...
    # 1. List of backends to try (if one fails, try the next)
    # 'api' is standard, 'html' mimics a browser, 'lite' is lightest
    ...
    for backend in backends:
        try:
            ...
            if results:
                # Check for junk results (bot detection pages)
                first_title = results[0].get('title', '').lower()
                if "google chrome" in first_title or "browser" in first_title:
                    print(f"⚠️ Backend {backend} returned junk. Skipping.")
                    results = []
                    continue # Try next backend
                
                print(f"✅ Success with backend: {backend}")
                break # We got good results!
        except Exception as e:
            print(f"❌ Backend {backend} failed: {e}")
            continue

    if not results:
        return "No search results found. The server IP might be temporarily blocked by DuckDuckGo."
...
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            # Filter args to only pass 'query' or 'prompt'
            valid_args = {}
            if "query" in t["args"]: valid_args["query"] = t["args"]["query"]
            if "prompt" in t["args"]: valid_args["prompt"] = t["args"]["prompt"]
            
            # If no valid args found but tool called, use original (fallback)
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        RULES:
        1. If the user asks about a person, university, or event, you MUST call 'web_search_tool'.
