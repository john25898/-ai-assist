...
# ==============================================================================
# 2. DEFINE THE TOOLS (SMART & ROBUST)
# ==============================================================================
...
# --- SMART ANTI-BOT SEARCH TOOL ---
@tool
def web_search_tool(query: str) -> str:
    """
    Use this tool to find real-time information, news, weather, specific people (VCs, CEOs), or universities.
    """
    ...
    try:
        # STRATEGY 1: Use 'lite' backend. 
        # This is an HTML-only version of DDG that is much harder to block.
        ...
        # STRATEGY 3: Junk Filter
        # If the result is just "Download Chrome" or "Google Translate", ignore it.
        valid_results = []
        for r in results:
            title = r.get('title', '').lower()
            body = r.get('body', '').lower()
            
            # Filter out common bot-trap pages
            if "download google chrome" in title or "google translate" in title or "browser" in title:
                continue
                
            valid_results.append(f"Title: {r.get('title')}\nLink: {r.get('href')}\nSnippet: {r.get('body')}\n")

        if not valid_results:
            return "Search performed, but results were blocked by bot protection. Please try a different query."
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        RULES:
        1. If asked about specific people (e.g., VCs, CEOs), universities, or current news, YOU MUST USE 'web_search_tool'.
