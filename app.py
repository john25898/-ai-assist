...
def ui_builder_tool(prompt: str) -> str:
    """
    Generates HTML, CSS, and React code.
    Args:
        prompt: Description of the UI to build.
    """
    messages = [
...
def backend_builder_tool(prompt: str) -> str:
    """
    Generates Python, Flask, or SQL code.
    Args:
        prompt: Description of the backend logic.
    """
    messages = [
...
# --- CRITICAL FIX: Simplified Signature (No **kwargs) ---
@tool
def web_search_tool(query: str) -> str:
    """
    Finds real-time information, news, weather, or facts on the internet.
    Args:
        query: The search string (e.g., 'weather in Nairobi').
    """
    ...
    try:
        # STRATEGY 1: Lite Backend (Best for bots)
        results = DDGS().text(query, max_results=5, backend="lite")
        
        # STRATEGY 2: Standard Backend (Fallback)
        if not results:
        ...
        if not valid_results:
            return "Search results were blocked by bot protection."
...
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            ...
            # Extract only the specific argument we expect
            valid_args = {}
            if "query" in t["args"]: valid_args["query"] = t["args"]["query"]
            elif "prompt" in t["args"]: valid_args["prompt"] = t["args"]["prompt"]
            
            # Fallback: If model sent 'location' or other junk, ignore it and use what we have
            if not valid_args and t["args"]:
                # Just grab the first value found
                first_val = next(iter(t["args"].values()))
                if t["name"] == "web_search_tool": valid_args["query"] = first_val
                else: valid_args["prompt"] = first_val
...
def handle_ask():
    ...
    2. 'complex': Requests for code, weather, news, people, or events.
    ...
    if "simple" in classification:
        ...
        # Simplified System Prompt to avoid confusing the model
        agent_system_message = SystemMessage(content="""
        You are a helpful AI assistant.
        For current info (people, news, weather), use 'web_search_tool'.
        For code, use the builder tools.
        """)
