...
# FIX: Use the stable community import for Tavily
...
try:
    ...
    # Note: TAVILY_API_KEY is checked automatically by the tool when used
...
# --- PROFESSIONAL TAVILY SEARCH TOOL (STABLE) ---
@tool
def web_search_tool(query: str) -> str:
    ...
    if not os.environ.get('TAVILY_API_KEY'):
        return "Error: TAVILY_API_KEY is missing in Render Environment Variables. Please add it."

    if not query or len(query.strip()) < 2: 
        return "Error: Empty search query."

    try:
        # Tavily handles the scraping and parsing automatically
        ...
        # Format the results clearly so the AI can read them
...
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            ...
            valid_args = t["args"]
            # Fallback for common hallucination
...
workflow.add_edge("tools", "brain") # Loop back to brain to summarize results
...
if db_url and db_url.startswith("postgresql"):
    ...
    pool = ConnectionPool(conninfo=db_url, min_size=0, max_size=2, kwargs=connection_kwargs)
...
def handle_ask():
    ...
    # Use system prompts to guide routing
    router_system = "Classify: 1. 'simple' (jokes, greetings). 2. 'complex' (search, code, specific entities/people). Return ONLY 'simple' or 'complex'."
    try:
    ...
    if "simple" in classification:
        ...
        # Force tool usage for facts
        agent_system_message = SystemMessage(content="""
        You are a helpful AI assistant.
        RULES:
        1. Use 'web_search_tool' for current events, weather, or specific people.
        2. Use 'ui_builder_tool' or 'backend_builder_tool' for coding.
        3. Summarize search results into a natural answer.
        """)
        ...
        try:
            ...
            # Check if it was a Tavily key error
            if "TAVILY_API_KEY" in str(e):
                final_answer = "I need a TAVILY_API_KEY in the server settings to perform searches."
            else:
