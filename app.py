...
def tool_node(state):
    ...
    # FORCE STOP: Add an AIMessage to stop the loop
...
# FORCE STOP: Tools go to END to prevent recursion loops
...
# --- CALLBACK (FIXED FOR LOGIN/ID ISSUES) ---
@app.route('/callback')
def callback():
    try:
        ...
        # 1. Try to find user by Auth0 ID (Standard)
        user = User.query.filter_by(auth0_id=auth0_id).first()
        
        if not user:
            # 2. Try to find user by Email (Prevent Duplicate Key Error)
            user = User.query.filter_by(email=email).first()
            
            if user:
                # User exists, just update the ID to match current login method
        ...
        # Important: Rollback to prevent database locks
        db.session.rollback()
        return redirect(url_for('index'))
