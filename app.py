...
def web_search_tool(query: str, **kwargs) -> str:
    ...
    try:
        # Use 'lite' backend to avoid bot blocks
        ...
        if not valid_results: return "Search blocked by anti-bot."
...
# ==============================================================================
# 3. BUILD THE AGENT GRAPH (WITH SUMMARIZER)
# ==============================================================================
...
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            ...
            if "prompt" in t["args"]: valid_args["prompt"] = t["args"]["prompt"]
            if not valid_args: valid_args = t["args"]
...
# NODE 3: The Summarizer (NEW! Prevents Loops, Professionalizes Output)
def summarizer_node(state):
    """
    Takes the tool output and writes a final, natural answer to the user.
    Does NOT use tools, so it cannot loop.
    """
    messages = state["messages"]
    # We construct a prompt for the final answer
    final_prompt = [
        SystemMessage(content="You are a helpful assistant. You just received data from a tool. Read the tool output and answer the user's original question naturally and professionally. If it was code, just present the code. If it was a search, summarize the answer."),
        *messages # Pass conversation history including tool outputs
    ]
    # Invoke simple Groq model (no tools bound)
    response = groq_llm.invoke(final_prompt)
...
workflow.add_node("summarizer", summarizer_node) # Add the new node
...
# Tools -> Summarizer (This is the safety valve)
workflow.add_edge("tools", "summarizer")

# Summarizer -> END (Guarantees the loop stops)
...
def handle_ask():
    ...
    IMPORTANT: Questions about "Who is...", "What is the...", or specific entities are ALWAYS 'complex'.
    ...
    if "simple" in classification:
        ...
        2. Do not say "I don't know". Use the search tool to find the answer.
        """)
