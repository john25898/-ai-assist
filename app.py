...
# REMOVED: from langchain_community.tools import DuckDuckGoSearchRun (This was breaking)
...
# --- FIXED SEARCH TOOL (Direct Implementation) ---
@tool
def web_search_tool(query: str) -> str:
    ...
    """
    try:
        # Use the direct library instead of LangChain wrapper
        results = DDGS().text(query, max_results=3)
        if not results:
            return "No search results found."
        
        # Format the results nicely for the AI to read
        formatted_results = "\n\n".join([
            f"Title: {r['title']}\nSnippet: {r['body']}\nLink: {r['href']}" 
            for r in results
        ])
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        # Complex path: Agent with Tools
        config = {"configurable": {"thread_id": user_id}}
        config["recursion_limit"] = 50
        
        # Force the agent to use tools
