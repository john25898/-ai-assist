...
# --- DATABASE STABILITY (USER DB - SQLAlchemy) ---
app.config['SQLALCHEMY_ENGINE_OPTIONS'] = {
    "pool_pre_ping": True,  # Pings DB before every query
...
def web_search_tool(query: str, **kwargs) -> str:
    ...
    try:
        # 1. Try Lite Backend (Best for bots/servers)
        results = DDGS().text(query, max_results=5, backend="lite")
        
        # 2. Fallback to Standard
        if not results:
             print("⚠️ Lite search failed, retrying...")
...
# --- DATABASE CONNECTION (CHAT HISTORY) ---
...
if db_url and db_url.startswith("postgresql"):
    ...
    # CRITICAL FIX: 'check' forces a ping before use
    pool = ConnectionPool(
        conninfo=db_url, 
        min_size=0,
        max_size=5,  # Allow multi-user
        max_lifetime=300, 
        max_idle=60,
        check=ConnectionPool.check_connection, # <--- THIS PREVENTS SSL ERRORS
        reconnect_timeout=5,
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        try:
            ...
            final_answer = "I'm having trouble connecting to memory. Please try again."
