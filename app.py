...
# Node 2: The Tools (FIXED TO PREVENT LOOPS)
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            ...
            # Add the ToolMessage (required by the graph)
            results.append(ToolMessage(tool_call_id=t['id'], name=t['name'], content=str(output)))
            
            # Accumulate output for the final answer
            final_content += str(output) + "\n\n"
            
    # --- THE FIX: Add an AIMessage immediately ---
    # This tricks the chat into thinking the AI responded, and allows us to end the graph.
...
# --- THE FIX: Force the tools to go to END, not back to Brain ---
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        # We increase recursion limit just in case, but the logic fix above should solve it
