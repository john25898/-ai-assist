...
def web_search_tool(query: str) -> str:
    ...
    YOU MUST PROVIDE A SPECIFIC QUERY string.
    """
    print(f"🔎 WEB SEARCH TOOL CALLED WITH QUERY: '{query}'") # Debug Log

    if not query or len(query.strip()) < 2:
        return "Error: You sent an empty search query. Please provide a specific topic to search for."

    try:
        # region="wt-wt" means "World Wide" (prevents local redirect issues)
        results = DDGS().text(query, max_results=4, region="wt-wt")
        
        if not results:
            # Fallback: Try again without region if it fails
...
def tool_node(state):
    ...
    for t in tool_calls:
        ...
        if selected_tool:
            print(f"🛠️ EXECUTING TOOL: {t['name']} with args: {t['args']}") # Debug Log
...
def handle_ask():
    ...
    if "simple" in classification:
        ...
        3. Do NOT run the tool if the query is empty.
        """)
